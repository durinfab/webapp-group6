...
document.getElementById("retrieveAndListAll")
    .addEventListener("click", function () {
        ...
        for (let key of Object.keys(Person.instances)) {
            const author = Person.instances[key];
...
createFormEl.authorId.addEventListener("input", function () {
    createFormEl.authorId.setCustomValidity(
        Person.checkActorIdAsId(createFormEl.authorId.value).message);
});
...
createFormEl["commit"].addEventListener("click", function () {
    ...
    createFormEl.authorId.setCustomValidity(
        Person.checkActorIdAsId(slots.authorId).message);
